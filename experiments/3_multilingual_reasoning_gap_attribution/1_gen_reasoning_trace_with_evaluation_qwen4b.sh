...
for SEED in "${SEED_LIST[@]}"; do
    ...
    --dataset_type polymath --eval_langs en,de,es,ar,ja,ko,th,bn,sw,te --seed ${SEED} --polymath_split low \
    --fasttext_model_path ${FASTTEXT_MODEL_PATH} --save_base_dir ${SAVE_BASE_DIR}

    python -m rlm_analysis.generate_reasoning_trace_with_evaluation --model_name ${MODEL_NAME} \
    --dataset_type polymath --eval_langs en,de,es,ar,ja,ko,th,bn,sw,te --seed ${SEED} --polymath_split medium \
    --fasttext_model_path ${FASTTEXT_MODEL_PATH} --save_base_dir ${SAVE_BASE_DIR}

    python -m rlm_analysis.generate_reasoning_trace_with_evaluation --model_name ${MODEL_NAME} \
    --dataset_type polymath --eval_langs en,de,es,ar,ja,ko,th,bn,sw,te --seed ${SEED} --polymath_split high \
    --fasttext_model_path ${FASTTEXT_MODEL_PATH} --save_base_dir ${SAVE_BASE_DIR}

    python -m rlm_analysis.generate_reasoning_trace_with_evaluation --model_name ${MODEL_NAME} \
    --dataset_type mmlu_prox_lite --eval_langs en,de,es,ar,ja,ko,th,bn,sw,te --seed ${SEED} \
    --fasttext_model_path ${FASTTEXT_MODEL_PATH} --save_base_dir ${SAVE_BASE_DIR}
done
