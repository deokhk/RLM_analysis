...
import regex
from rlm_analysis.eval.bundled.latex2sympy2.latex2sympy2 import latex2sympy
from rlm_analysis.eval.bundled.symeval.core import EvaluatorMath
...
# Taken from "PolyMath" github (https://github.com/QwenLM/PolyMath/tree/main)
...
def math_equal(
    ...
    # Make sure to remove "\\displaystyle" in prediction, and normalize "\\dfrac" to "\\frac"
    prediction = str(prediction).replace("\\displaystyle", "").replace("\\dfrac", "\\frac")
        ...
        if is_digit(prediction) and is_digit(reference):
            ...
            for item in gt_result:
                try:
                    ...
                    continue
            return False
        elif is_digit(reference) and not is_digit(prediction):
            # e.g. reference is 0.5, but some models output 0.5\\text{minutes}
            prediction_str = str(prediction)
            prediction_str = strip_string(prediction_str, skip_unit=True)

            prediction = parse_digits(prediction_str)
            reference = parse_digits(reference)
            # number questions
            if include_percentage:
                gt_result = [reference / 100, reference, reference * 100]
